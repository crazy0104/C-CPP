...
void aes_encrypt(aes_context *ctx, const unsigned char in[16], unsigned char *out)
{
    const uint32_t *rk;
...
void aes_decrypt(aes_context *ctx, const unsigned char in[], unsigned char *out)
{
    const uint32_t *rk;
    uint32_t s0, s1, s2, s3, t0, t1, t2, t3;
#ifndef FULL_UNROLL
...
{
    if (length == 0) {
        return CRYPTO_ERR_AES_INVALID_INPUT_LENGTH;
    }
    while (length) {
